...
-- ==================================================
-- [추가 기능] 즐겨찾기 채팅방 및 초대 수락/거절 프로세스
-- ==================================================

-- 1. CHAT_ROOM_USER 테이블에 즐겨찾기 컬럼 추가
ALTER TABLE CHAT_ROOM_USER ADD IS_FAVORITE NUMBER(1) DEFAULT 0;
ALTER TABLE CHAT_ROOM_USER ADD CONSTRAINT CHK_IS_FAVORITE CHECK (IS_FAVORITE IN (0, 1));
COMMENT ON COLUMN CHAT_ROOM_USER.IS_FAVORITE IS '즐겨찾기 여부 (0:OFF, 1:ON)';

-- 2. CHAT_ROOM_USER 테이블에 초대 상태 컬럼 추가
ALTER TABLE CHAT_ROOM_USER ADD INVITATION_STATUS VARCHAR2(20) DEFAULT 'ACCEPTED';
ALTER TABLE CHAT_ROOM_USER ADD CONSTRAINT CHK_INVITATION_STATUS 
CHECK (INVITATION_STATUS IN ('PENDING', 'ACCEPTED', 'REJECTED'));
COMMENT ON COLUMN CHAT_ROOM_USER.INVITATION_STATUS IS '초대 상태 (PENDING:대기, ACCEPTED:수락, REJECTED:거절)';

-- 3. 시스템 계정 추가 (시스템 메시지용)
INSERT INTO MEMBER (MEMBER_ID, LOGIN_ID, PASSWORD, NAME, STATUS, IS_ONLINE, CREATED_AT) 
VALUES (1, 'system', 'system', '시스템', 'ACTIVE', 0, SYSDATE);

-- 4. CHAT_MESSAGE 테이블에 SYSTEM 메시지 타입 추가
ALTER TABLE CHAT_MESSAGE DROP CONSTRAINT CHK_MSG_TYPE;
ALTER TABLE CHAT_MESSAGE ADD CONSTRAINT CHK_MSG_TYPE 
CHECK (MESSAGE_TYPE IN ('TEXT', 'IMAGE', 'EMOJI', 'ENTER', 'LEAVE', 'SYSTEM'));

-- 5. CHAT_ROOM 테이블에 메시지 카운트 컬럼 추가 (unreadCount 계산용)
ALTER TABLE CHAT_ROOM ADD TOTAL_MESSAGE_COUNT NUMBER DEFAULT 0;
COMMENT ON COLUMN CHAT_ROOM.TOTAL_MESSAGE_COUNT IS '총 메시지 수 (읽지 않은 메시지 카운트 계산용)';

-- 6. 공지 메시지 기능용 컬럼 추가
ALTER TABLE CHAT_ROOM ADD NOTICE_MESSAGE_ID NUMBER;
ALTER TABLE CHAT_ROOM ADD NOTICE_CONTENT CLOB;
COMMENT ON COLUMN CHAT_ROOM.NOTICE_MESSAGE_ID IS '고정된 공지 메시지 ID';
COMMENT ON COLUMN CHAT_ROOM.NOTICE_CONTENT IS '공지 메시지 내용';

-- 7. CHAT_ROOM_USER 테이블에 마지막 읽은 메시지 카운트 추가
ALTER TABLE CHAT_ROOM_USER ADD LAST_READ_MESSAGE_COUNT NUMBER DEFAULT 0;
COMMENT ON COLUMN CHAT_ROOM_USER.LAST_READ_MESSAGE_COUNT IS '마지막으로 읽은 메시지 카운트';

-- 8. CHAT_ROOM_USER 테이블에 역할 컬럼 추가 (방장, 관리자, 일반 멤버)
ALTER TABLE CHAT_ROOM_USER ADD ROLE VARCHAR2(20) DEFAULT 'MEMBER';
ALTER TABLE CHAT_ROOM_USER ADD CONSTRAINT CHK_ROLE CHECK (ROLE IN ('OWNER', 'ADMIN', 'MEMBER'));
COMMENT ON COLUMN CHAT_ROOM_USER.ROLE IS '채팅방 내 역할 (OWNER:방장, ADMIN:관리자, MEMBER:일반멤버)';

-- 9. CHAT_MESSAGE 테이블에 부모 메시지 ID 추가 (답장 기능용)
ALTER TABLE CHAT_MESSAGE ADD PARENT_MESSAGE_ID NUMBER;
ALTER TABLE CHAT_MESSAGE ADD CONSTRAINT FK_MSG_PARENT 
FOREIGN KEY (PARENT_MESSAGE_ID) REFERENCES CHAT_MESSAGE(MESSAGE_ID) ON DELETE SET NULL;
COMMENT ON COLUMN CHAT_MESSAGE.PARENT_MESSAGE_ID IS '답장 대상 메시지 ID (답장 기능용)';

-- 10. CHAT_ROOM 테이블에 버전 컬럼 추가 (낙관적 락킹용)
ALTER TABLE CHAT_ROOM ADD VERSION NUMBER DEFAULT 0;
COMMENT ON COLUMN CHAT_ROOM.VERSION IS '낙관적 락킹용 버전 번호';
